...
class EventsController {
  ...
  async getAttendanceAudit(req, res) {
    try {
      const { uid } = req.params;
      const { participantId, limit, before } = req.query;

      // Parse and validate parameters
      const parsedParticipantId = participantId ? parseInt(participantId, 10) : null;
      const parsedLimit = limit ? parseInt(limit, 10) : 100;
      
      // Ensure limit is a valid number
      const validLimit = isNaN(parsedLimit) ? 100 : parsedLimit;

      const entries = await eventRepository.getAttendanceAudit(
        uid, 
        parsedParticipantId && !isNaN(parsedParticipantId) ? parsedParticipantId : null, 
        validLimit, 
        before || null
      );
   ...
   * Admin/staff get full report, event creators get summary only
   */
  async getAnalyticsReport(req, res) {
    try {
      ...
      if (event.status !== 'completed') {
        return res.status(400).json({ 
          success: false, 
          message: 'Analytics report is only available for completed events' 
        });
      }

      // Check permissions: admin/staff can view all, event creators can view their own
      const isAdminOrStaff = userRole === 'admin' || userRole === 'staff';
      const isEventCreator = event.created_by_user_id === userId;

      if (!isAdminOrStaff && !isEventCreator) {
        return res.status(403).json({ 
          success: false, 
      ...
      // Get report (full for admin/staff, summary for creators)
      let report;
      if (isAdminOrStaff) {
        report = await eventReportService.getEventReport(uid);
      } else {
        report = await eventReportService.getEventReportSummary(uid);
      }

      if (!report) {
        return res.status(404).json({ 
          success: false, 
          message: 'No analytics report found for this event. It may still be generating.' 
        });
      }
      ...
      res.json({
        success: true,
        data: report,
        access_level: isAdminOrStaff ? 'full' : 'summary'
      });
    } catch (error) {
   ...
   */
  async regenerateAnalyticsReport(req, res) {
    try {
      const { uid } = req.params;
      const userId = req.currentUserId;
      ...
      if (event.status !== 'completed') {
        return res.status(400).json({ 
          success: false, 
          message: 'Analytics report can only be generated for completed events' 
        });
      }

      // Generate report
      const report = await eventReportService.generateEventReport(uid, userId, { isAutoGenerated: false });

      res.json({
        success: true,
        message: 'Analytics report regenerated successfully',
        data: report
      });
    } catch (error) {
      console.error('Regenerate analytics report error:', error);
      res.status(500).json({ 
        success: false, 
        message: 'Failed to regenerate analytics report', 
        error: error.message 
      });
    }
  }
   ...
   */
  async downloadAnalyticsReportPdf(req, res) {
    try {
      const { uid } = req.params;
      ...
      // Get report data
      const report = await eventReportService.getEventReport(uid);
      if (!report) {
        return res.status(404).json({ 
          success: false, 
          message: 'No analytics report found. Please regenerate the report first.' 
        });
      }

      // Generate PDF
      const pdfPath = pdfReportService.getTempPdfPath(uid);
      await pdfReportService.generateReportPDF(report, pdfPath);

      // Send file
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="event-report-${uid}.pdf"`);
      ...
      stream.on('end', () => {
        pdfReportService.cleanupTempPdf(pdfPath);
      });
      ...
      console.error('Download analytics report PDF error:', error);
      res.status(500).json({ 
        success: false, 
