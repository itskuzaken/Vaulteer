    ...
    for (const row of rows) {
      try {
        ...
        if (updated && updated.status === 'completed' && prevStatus !== 'completed') {
          ...
          // Auto-generate analytics report for completed event
          try {
            const report = await eventReportService.generateEventReport(row.uid, null, { isAutoGenerated: true });
            console.log(`[EventCompletionScheduler] Generated analytics report ${report.report_id} for event ${row.uid}`);
          } catch (reportErr) {
