...
async function getByUid(uid) {
  ...
  const [rows] = await pool.query(
    `SELECT u.user_id AS id, u.uid, u.name, u.email, r.role, u.status, u.date_added
...
async function update(id, { name, email, status }) {
  ...
  const [rows] = await pool.query(
    `SELECT u.user_id AS id, u.name, u.email, r.role, u.status, u.date_added FROM users u JOIN roles r ON u.role_id = r.role_id WHERE u.user_id = ?`,
    [id]
  );
  return rows[0] || null;
}
