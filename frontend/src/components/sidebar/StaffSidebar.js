...
export default function Navigation({
  ...
  const [selectedMenu, setSelectedMenu] = useState(null); // Track the currently selected menu
  const [selectedSubMenu, setSelectedSubMenu] = useState(null); // Track the selected sub-menu
  const [showLogoutModal, setShowLogoutModal] = useState(false); // Track logout modal visibility

  const toggleMenu = (menu) => {
    setSelectedMenu((prev) => (prev === menu ? null : menu)); // Toggle the selected menu
  };
...
/* Add this CSS to your global styles (e.g., globals.css or tailwind.config.js if using Tailwind) */
/*
